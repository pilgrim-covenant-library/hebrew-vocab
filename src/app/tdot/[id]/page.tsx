...
import { TDOTBadge, TDOTDisplay } from '@/components/tdot';
...
export default function TDOTDetailPage() {
  ...
  useEffect(() => {
    ...
    if (entry) {
      ...
      if (!viewed.includes(entry.id)) {
        ...
        storage.set('tdot-viewed', updated);
      }
    }
  }, [entry]);
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      title: `TDOT: ${entry.hebrew} (${entry.transliteration})`,
      text: `Theological study of ${entry.hebrew} - ${entry.shortDef}`,
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-4 py-6 max-w-2xl">
        {/* Category Badge */}
        <div className="mb-4">
          <TDOTBadge category={entry.category} size="md" />
        </div>

        {/* Hebrew Word */}
        <div className="mb-4">
          <h1 className="font-hebrew text-4xl text-foreground" dir="rtl">{entry.hebrew}</h1>
        ...
        <nav className="mt-12 pt-6 border-t flex items-center justify-between">
          {prevEntry ? (
            <Link
              href={`/tdot/${prevEntry.id}`}
              className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
              <ChevronLeft className="w-4 h-4" />
              <span className="hidden sm:inline font-hebrew" dir="rtl">{prevEntry.hebrew}</span>
          ...
          {nextEntry ? (
            <Link
              href={`/tdot/${nextEntry.id}`}
              className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground transition-colors"
            >
              <span className="hidden sm:inline font-hebrew" dir="rtl">{nextEntry.hebrew}</span>
