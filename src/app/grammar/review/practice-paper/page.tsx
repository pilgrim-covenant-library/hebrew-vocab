...
export default function PracticePaperPage() {
  ...
  // VA: matching change
  const handleMatchingChange = (qId: string, hebrew: string, category: string) => {
    if (vaRevealed[qId]) return;
    setVaMatching(prev => ({ ...prev, [qId]: { ...(prev[qId] || {}), [hebrew]: category } }));
  };
  ...
  const handleCheckVA = () => {
    ...
    for (const pair of q.matchingPairs) {
      if (m[pair.hebrew] === pair.category) matchCorrect++;
    }
  ...
  const finalScores = useMemo(() => {
    ...
    for (const q of vaInPaper) {
      ...
      for (const pair of q.matchingPairs) {
        matchingTotal++;
        if (m[pair.hebrew] === pair.category) matchingCorrect++;
      }
    }
      ...
      <main className="flex-1 container mx-auto px-4 py-4 max-w-2xl">
            ...
            <div>
              <Card className="mb-4">
                <CardContent className="py-5 text-center">
                  ...
                  {q.hebrew && <HebrewWord hebrew={q.hebrew} size="xl" />}
                </CardContent>
              </Card>
        ...
        {current?.type === 'va' && (() => {
          ...
          const matchingResults = revealed ? q.matchingPairs.map(pair => {
            const student = matching[pair.hebrew] || '';
            return { hebrew: pair.hebrew, correct: pair.category, student, isCorrect: student === pair.category };
          }) : null;
          ...
          return (
            <div className="space-y-5">
              ...
              <p className="text-2xl text-center leading-relaxed" dir="rtl" lang="he">{q.hebrew}</p>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {q.matchingPairs.map((pair) => {
                    const studentChoice = matching[pair.hebrew] || '';
                    ...
                    return (
                      <div key={pair.hebrew} className={cn(
                        ...
                        <span className="font-serif text-lg shrink-0 min-w-[120px] font-medium">{pair.hebrew}</span>
                        <span className="text-muted-foreground shrink-0">&rarr;</span>
                        {!revealed ? (
                          <select
                            value={studentChoice}
                            onChange={(e) => handleMatchingChange(q.id, pair.hebrew, e.target.value)}
