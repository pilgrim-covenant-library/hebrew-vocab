  ...
  const filteredWords = useMemo(() => {
    return words.filter((word) => {
      ...
      if (search) {
        ...
        const matchesHebrew = word.hebrew.toLowerCase().includes(searchLower);
        ...
        if (!matchesHebrew && !matchesGloss && !matchesTranslit) {
          return false;
        }
      }
          ...
          {filteredWords.map((word) => {
            ...
            return (
              <Card
                ...
                aria-label={`${word.hebrew} - ${word.gloss}. ${isExpanded ? 'Press to collapse' : 'Press to expand details'}`}
              >
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t space-y-3">
                      ...
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        <span>Frequency: {word.frequency}x in Hebrew Bible</span>
                        {progress[word.id] && (
                          <span>
