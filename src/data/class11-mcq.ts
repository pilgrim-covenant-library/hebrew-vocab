...
const ch30MemoryQuestions: PracticeQuestion[] = [
  ...
  { id: 'class11-memory-ch30-vs-hiphil', prompt: 'What separates the Piel from the Hiphil at a glance?', options: ['the Piel has a נ prefix', 'the Piel doubles the FIRST root consonant', 'the Piel has no prefix at all', 'the Piel doubles the SECOND root consonant, while the Hiphil shows a ה prefix and a Hireq Yod'], correctIndex: 3, explanation: 'Ask two questions of any unfamiliar form: is the middle root consonant doubled, and is there a Hireq Yod? The answers separate the two most common derived stems.', category: 'ch30-memory' },
  { id: 'class11-memory-ch30-mem', prompt: 'Which stems use a מ prefix on their participles?', options: ['Piel, Pual, Hiphil, Hophal, and Hithpael — all the derived stems except the Niphal', 'only the Piel', 'only the Piel and Pual', 'none of them'], correctIndex: 0, explanation: 'The Niphal participle keeps its נ (נִקְטָל). Everywhere else a מ prefix signals "derived stem," and the vowels tell you which one.', category: 'ch30-memory' },
...
const ch31Questions: PracticeQuestion[] = [
  ...
  { id: 'class11-ch31-8', prompt: 'Which three features remain the Piel signature across the weak classes?', options: ['a ה prefix, a Hireq Yod, and a Pathach', 'a Daghesh Forte in the second root consonant, Pathach under the first (except in the Perfect), and Shewa under Imperfect preformatives', 'a נ prefix, a Qibbuts, and a Qamets', 'a Qibbuts, a Shureq, and a Tsere'], correctIndex: 1, explanation: 'Learn those three once and the whole Piel — strong and weak — becomes readable, with II-Guttural roots as the single recurring exception.', category: 'ch31' },
];

const ch31MemoryQuestions: PracticeQuestion[] = [
  { id: 'class11-memory-ch31-berakh', prompt: 'Which root would you look up for this Piel form?', hebrew: 'בֵּרַךְ', transliteration: 'berakh', options: ['בָּרַךְ ("to bless") — ר rejects the Daghesh Forte, so the vowel lengthens', 'בָּרָא ("to create")', 'בָּחַר ("to choose")', 'בְּרִית ("covenant")'], correctIndex: 0, explanation: 'Compensatory lengthening is why the expected Hireq appears as a Tsere. The Pual of the same root is בֹּרַךְ, with a Holem for the same reason.', category: 'ch31-memory' },
  { id: 'class11-memory-ch31-nicham', prompt: 'Which root would you look up for this Piel form?', hebrew: 'נִחַם', transliteration: 'nicham', options: ['נָחָה ("to rest")', 'נָחַם ("to be sorry; to comfort") — a II-Guttural verb', 'לָחַם ("to fight")', 'חָמַם ("to be hot")'], correctIndex: 1, explanation: 'Careful: the נ here is a ROOT letter, not the Niphal stem prefix. The Daghesh-less ח with virtual doubling gives the Piel away.', category: 'ch31-memory' },
  { id: 'class11-memory-ch31-tsivvah', prompt: 'Which root would you look up for this Piel form?', hebrew: 'צִוָּה', transliteration: 'tsivvah', options: ['צוּר ("rock")', 'צַר ("adversary")', 'צָוָה ("to command") — a III-ה verb', 'יָצָא ("to go out")'], correctIndex: 2, explanation: 'The Daghesh Forte sits in the ו, the second root consonant, and the Qamets-He ending marks the III-ה class.', category: 'ch31-memory' },
...
const ch32MemoryQuestions: PracticeQuestion[] = [
  ...
  { id: 'class11-memory-ch32-vs-piel', prompt: 'קִטֵּל and קֻטַּל share their consonants. What differs?', hebrew: 'קִטֵּל / קֻטַּל', options: ['the root', 'the person', 'the number', 'the first vowel — and with it the voice: active Piel versus passive Pual'], correctIndex: 3, explanation: 'One vowel carries the whole difference between "he smashed" and "it was smashed." In the Piel-Pual pair, the vowels do all the work.', category: 'ch32-memory' },
  { id: 'class11-memory-ch32-vs-hophal', prompt: 'How do you tell the Pual יְקֻטַּל from the Hophal יֻקְטַל?', hebrew: 'יְקֻטַּל / יֻקְטַל', options: ['by the root', 'by WHERE the u-class vowel sits: under the first root consonant (Pual) or under the preformative (Hophal)', 'by the person', 'they cannot be distinguished'], correctIndex: 1, explanation: 'The Pual also doubles the second root consonant, which the Hophal never does. Two checks, both quick.', category: 'ch32-memory' },
...
const ch33MemoryQuestions: PracticeQuestion[] = [
  { id: 'class11-memory-ch33-mevorakh', prompt: 'Which root would you look up for this Pual form?', hebrew: 'מְבֹרָךְ', transliteration: 'mevorakh', options: ['בָּרַךְ ("to bless") — ר rejects the Daghesh Forte, so the Qibbuts lengthens to Holem', 'בָּרָא ("to create")', 'בָּחַר ("to choose")', 'מָרַךְ, an unrelated root'], correctIndex: 0, explanation: 'The same compensatory lengthening you met in the Piel בֵּרַךְ, one vowel class over: Hireq to Tsere in the Piel, Qibbuts to Holem in the Pual.', category: 'ch33-memory' },
  { id: 'class11-memory-ch33-yekhullu', prompt: 'Which root would you look up for this Pual form?', hebrew: 'וַיְכֻלּוּ', transliteration: 'vaykhullu', options: ['כּוּן ("to be established")', 'כָּלָה ("to be complete, finished") — a III-ה verb', 'כָּסָה ("to cover")', 'יָכֹל ("to be able")'], correctIndex: 1, explanation: 'Genesis 2:1: "and the heavens and the earth were finished." The Qibbuts and the Daghesh Forte in the ל are both intact.', category: 'ch33-memory' },
  { id: 'class11-memory-ch33-nuchamah', prompt: 'Which root would you look up for this Pual form?', hebrew: 'נֻחָמָה', transliteration: 'nuchamah', options: ['נוּחַ ("to rest")', 'לָחַם ("to fight")', 'נָחַם ("to comfort") — a II-Guttural verb', 'חָמַם ("to be hot")'], correctIndex: 2, explanation: 'The ח cannot take the Daghesh Forte, so the form shows the guttural adjustment; the Qibbuts still marks the passive. Isaiah 54:11: "not comforted."', category: 'ch33-memory' },
...
export const CLASS11_WORD_GROUPS: PracticeQuestionGroup[] = [
  wordGroup({ id: 'class11-word-nagad', hebrew: 'נָגַד', transliteration: 'nagad', transliterationOptions: ['nagad', 'nagash', 'naga', 'nabat'], transliterationCorrectIndex: 0, meaning: '(Hiphil) to tell, announce, declare', meaningOptions: ['(Hiphil) to tell, announce, declare', 'to draw near', 'to touch', 'to look'], meaningCorrectIndex: 0 }),
  wordGroup({ id: 'class11-word-shalakh', hebrew: 'שָׁלַךְ', transliteration: 'shalakh', transliterationOptions: ['shalach', 'shalakh', 'shavat', 'shakhach'], transliterationCorrectIndex: 1, meaning: '(Hiphil) to throw, cast, send', meaningOptions: ['to send', '(Hiphil) to throw, cast, send', 'to cease', 'to forget'], meaningCorrectIndex: 1 }),
  ...
  wordGroup({ id: 'class11-word-yadah', hebrew: 'יָדָה', transliteration: 'yadah', transliterationOptions: ['yalad', 'yarad', 'yadah', 'yada'], transliterationCorrectIndex: 2, meaning: '(Hiphil) to thank, praise, confess', meaningOptions: ['to give birth', 'to go down', '(Hiphil) to thank, praise, confess', 'to know'], meaningCorrectIndex: 2 }),
  wordGroup({ id: 'class11-word-pala', hebrew: 'פָּלָא', transliteration: 'pala', transliterationOptions: ['panah', 'patach', 'palal', 'pala'], transliterationCorrectIndex: 3, meaning: '(Niphal) to be extraordinary, wonderful', meaningOptions: ['to turn', 'to open', 'to pray', '(Niphal) to be extraordinary, wonderful'], meaningCorrectIndex: 3 }),
