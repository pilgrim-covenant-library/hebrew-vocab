...
import { TDOTBadge } from './TDOTBadge';
import type { TDOTEntry } from '@/types';

interface TDOTCardProps {
  entry: TDOTEntry;
  className?: string;
}

export function TDOTCard({ entry, className }: TDOTCardProps) {
  return (
    <Link href={`/tdot/${entry.id}`} className="block group">
      <Card
        className={cn(
      ...
      >
        <CardContent className="p-4 sm:p-5">
          ...
          <div className="flex items-center justify-between gap-2 mb-3">
            <TDOTBadge category={entry.category} size="sm" />
          ...
          {/* Hebrew Word */}
          <div className="mb-2">
            <h3 className="font-hebrew text-2xl text-foreground" dir="rtl">{entry.hebrew}</h3>
...
export function TDOTCardSkeleton() {
  return (
