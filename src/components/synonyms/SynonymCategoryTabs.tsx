...
interface SynonymCategoryTabsProps {
  activeCategory: HebrewSynonymCategory | 'all';
  onCategoryChange: (category: HebrewSynonymCategory | 'all') => void;
  categoryCounts: Record<HebrewSynonymCategory | 'all', number>;
  className?: string;
}
...
export function SynonymCategoryTabs({
  ...
  const tabs: { key: HebrewSynonymCategory | 'all'; label: string }[] = [
    { key: 'all', label: 'All' },
    ...Object.entries(HEBREW_SYNONYM_CATEGORIES).map(([key, info]) => ({
      key: key as HebrewSynonymCategory,
      label: info.label,
    })),
  ];
